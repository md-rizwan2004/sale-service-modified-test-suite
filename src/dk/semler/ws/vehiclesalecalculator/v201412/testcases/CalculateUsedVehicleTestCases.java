...
public class CalculateUsedVehicleTestCases {
	...
	public void VehicleGrandTotalPriceInclVATShouldMatch() {
		assertEquals(new BigDecimal(expectedtotalpriceinclvat).setScale(2), 
				(responseMessage.getRESPONSE().getCALCULATION().getGRANDTOTALPRICE().getEXCLTAXINCLVAT() != null) ? 
						responseMessage.getRESPONSE().getCALCULATION().getGRANDTOTALPRICE().getEXCLTAXINCLVAT().setScale(2) : BigDecimal.ZERO);
	}
